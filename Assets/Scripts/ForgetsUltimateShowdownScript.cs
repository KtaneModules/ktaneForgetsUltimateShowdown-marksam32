...
public partial class ForgetsUltimateShowdownScript : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        if (_usedMethods.Select(x => x.Id).Contains(MethodId.SimonsStages))
        {
            Audio.PlaySoundAtTransform(
                _usedMethods.Select(x => x.Id).Contains(MethodId.AND)
                    ? SFX[rnd.Range(0, 2) == 0 ? 0 : 7].name
                    : SFX[0].name, Module.transform);
        }
        else
        {
    ...
    private MissionSettings GetMissionSetting()
    {
        var missionDesc = Application.isEditor
            ? MissionText
        ...
        var match = Constants.MissionRegex.Match(missionDesc);
        if (!match.Success)
        {
            _logger.LogMessage("Mission not parsed.");
            return null;
        }


        var parsedGroups = match.Groups[1].ToString().Split('|')
            .Select(x => x.Substring(1, x.Length - 2).ToLowerInvariant().Split(','))
            .ToList();
        ...
        foreach (var group in parsedGroups)
        {
            ...
            {
                if (!Constants.PossibleMissionMethods.Contains(item))
                {
                    _logger.LogMessage(
                        "Could not load mission settings, the specified encryption method {0} does not exist!", item);
                    return null;
                }
            }
        }

        var methods = parsedGroups
            .Select(x => x.Select(y => (MethodId)Constants.PossibleMissionMethods.IndexOf(y)).ToList()).ToList();
        var encryptionOrder = match.Groups[2].ToString().ToLowerInvariant() == "random"
            ? EncryptionOrder.Random
            : EncryptionOrder.Fixed;
        _logger.LogMessage("Mission information successfully parsed!");
        return new MissionSettings(methods, encryptionOrder);
    }

    private void InitMissionSettings()
    {
        ...
        _missionSettings.SelectedMethods.ForEach(x => methods.Add(x.PickRandom()));
        _usedMethods = _missionSettings.EncryptionOrder == EncryptionOrder.Fixed
            ? methods.Select(x => possible[(int)x]).ToList()
            : methods.Shuffle().Select(x => possible[(int)x]).ToList();
    }
    ...
    private void InitMethods()
    {
        var possible = new List<IFUSComponentSolver>
        {
            new ForgetMeNotComponent(), //0
            new ForgetMeLaterComponent(), //1
            new ANDComponent(), //2
            new ForgetMeNowComponent(), //3
            new ForgetInfinityComponent(), //4
            new ForgetEverythingComponent(), //5
            new SimonsStagesComponent(), //6
            new ForgetUsNotComponent() //7
        };
        var methods = new List<int>();
        for (var i = 0; i < 4; i++)
        {
            var method = rnd.Range(0, 8);
            while (methods.Contains(method) ||
                   (method.EqualsAny(1, 2, 7) && methods.Any(x => new[] { 1, 2, 7 }.Contains(x))))
            {
        ...
        if (methods.Contains(6) &&
            new[] { SimonsStagesColor.Blue, SimonsStagesColor.Lime }.Contains(simonStagesColor) &&
            methods.IndexOf(6) > 1)
        {
            var method = rnd.Range(0, 2);
            if (methods[method] == 7)
        ...
        _usedMethods = methods.Select(x => possible[x]).ToList();
    ...
    {
        var componentInfo = new List<ComponentInfo>();
        ...
        for (var i = 0; i < 4; i++)
        {
            ...
            switch (_usedMethods[i].Id)
            {
                case MethodId.ForgetMeNot:
                    ...
                    componentInfo.Add(new ComponentInfo());
                    break;
                case MethodId.ForgetInfinity:
                    ...
                    componentInfo.Add(new ComponentInfo { IgnoredNumbers = ignored });
                    obj.GetComponentInChildren<TextMesh>().text =
                        string.Format("{0} / {1}", ignored.Item1, ignored.Item2);
                    break;
                case MethodId.ForgetMeNow:
                    ...
                    _logger.LogMessage(
                        "The starting number is {0}, which causes the number used to start the module to be {1}.",
                        startingNumber.ToString("D2"), startingDigit);
                    componentInfo.Add(new ComponentInfo { StartingNumber = startingDigit });
                ...
                case MethodId.SimonsStages:
                    ...
                    componentInfo.Add(new ComponentInfo { SimonsStagesColor = chosenColor });
                ...
                case MethodId.ForgetMeLater:
                    var ruleNums = Constants.ForgetMeLaterRules.Shuffle().Take(12).ToList();
                    //testing purposes
                    ...
                    componentInfo.Add(new ComponentInfo { Rules = ruleNums });
                ...
                case MethodId.AND:
                    ...
                    componentInfo.Add(new ComponentInfo { LogicGates = logicGates });
                ...
                case MethodId.ForgetEverything:
                    ...
                    var rotation = Constants.ForgetEverythingPositions[i].PickRandom();
                    ...
                    componentInfo.Add(new ComponentInfo { FEColors = colors });
                    for (int j = 0; j < 3; j++)
                    {
                        obj.GetComponent<MeshRenderer>().materials[j + 1].color =
                            Constants.ForgetEverythingLedColors[(int)colors[j]];
                    }

                    break;
                case MethodId.ForgetUsNot:
                    var positions = Enumerable.Range(1, 12).ToList().Shuffle();
                    componentInfo.Add(new ComponentInfo { Positions = positions });
        ...
        for (var i = 0; i < 4; i++)
        {
            numberInfo.Number = _usedMethods[i].Encrypt(Info, componentInfo[i], numberInfo);
            _logger.LogMessage("After step {0}({1}) of the encryption, the number is: {2}", i + 1, _usedMethods[i].Name,
                numberInfo.Number);
            if (numberInfo.Number.Length != 12)
            {
                throw new InvalidOperationException(
                    "Invalid length of a number, please report this logfile to Marksam!");
            }
        }
    ...
    private void HandlePress(int i)
    {
        ...
        if (_presses.Join("") == _answer)
        {
            ...
            {
                Audio.PlaySoundAtTransform(
                    _usedMethods.Select(x => x.Id).Contains(MethodId.AND) ? SFX[rnd.Range(3, 5)].name : SFX[4].name,
                    Module.transform);
            }
            else
            {
    ...
    private IEnumerator ANDCycle(List<ANDLogicGate> gates, TextMesh textMesh)
    {
        while (true)
        {
            ...
            for (var i = 0; i < 12; i++)
            {
                textMesh.text = Constants.AndLogicGateChars[(int)gates[i]].ToString();
                yield return new WaitForSeconds(1f);
            }
